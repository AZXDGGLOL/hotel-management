...
public sealed class CustomerRepository
{
    ...
    public DataTable GetLookup()
    {
        const string sql = """
                           SELECT MemberId,
                                  Name + N' ' + LastName + N' (' + MemberId + N')' AS MemberLabel
                           FROM [Member]
    ...
    {
        const string sql = """
                           INSERT INTO [Member] (MemberId, NationalId, Name, LastName, Nationality, Address, Phone, Email)
                           VALUES (@MemberId, @NationalId, @Name, @LastName, @Nationality, @Address, @Phone, @Email)
                           """;
        return SqlDataAccess.Execute(sql, BuildParameters(customer));
    }
    ...
    private static SqlParameter[] BuildParameters(Customer customer)
    {
        return
        [
